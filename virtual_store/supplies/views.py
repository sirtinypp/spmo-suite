...
def profile(request):
    ...
    return render(request, 'supplies/profile.html', {'orders': my_orders})
...
def requisition_slip(request):
    ...
    if not pdf.err:
        ...
        response['Content-Disposition'] = 'attachment; filename="requisition_slip.pdf"'
        return response
...
def my_app_status(request):
    # Get the user's department (Placeholder: 'General')
    user_dept = request.session.get('user_department', 'General') 
